...
export const FileUpload = ({
  ...
  const handleFileChange = (newFiles: File[]) => {
    ...
    onChange(updatedFiles); // Pass all files, not just the new ones
  };

  const removeFile = (index: number) => {
    const updatedFiles = files.filter((_, i) => i !== index);
    setFiles(updatedFiles);
    onChange(updatedFiles); // Notify parent about the change
    setError("");
  };
    ...
    <div className="w-full" {...getRootProps()}>
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="relative z-20 text-sm text-neutral-500 dark:text-neutral-500 mt-1">
            {minFiles && `Upload at least ${minFiles} images`}
          </p>
